...
static void pmergeme_recursive(std::deque<unsigned>& d, std::size_t d_pos[], unsigned track);

static void pairing(std::vector<unsigned>& v, unsigned track);
static void pairing(std::deque<unsigned>& d, unsigned track);

static void insertion(std::vector<unsigned>& v, std::size_t d_pos[], unsigned track,
					  std::size_t b_len);
static void insertion(std::deque<unsigned>& d, std::size_t d_pos[], unsigned track,
					  std::size_t b_len);
...
static void binary_insertion(std::deque<unsigned>& v, std::size_t d_pos[], unsigned track,
							 std::deque<unsigned>::reverse_iterator rit_b,
...
void PmergeMe::sort(std::deque<unsigned>& deq) throw(std::bad_alloc) {
	...
	if (deq.size() > 2) {
		std::size_t max_k = static_cast<std::size_t>(
			std::ceil(std::log(deq.size() * 3.0l / 2) / std::log(2.0l)) - 1);
		max_k = ((1ul << (max_k + 1)) + 1 - 2 * (max_k & 1)) / 3
				- ((1ul << (max_k)) + 1 - 2 * ((max_k - 1) & 1)) / 3;
		d_pos = new std::size_t[max_k];
	} else
		d_pos = NULL;
	pmergeme_recursive(deq, d_pos, 1);
	delete[] d_pos;
}

static void pmergeme_recursive(std::vector<unsigned>& d, std::size_t d_pos[], unsigned track) {
	pairing(d, track);
	std::size_t b_len = d.size() / track / 2;
	if (b_len > 1)
		pmergeme_recursive(d, d_pos, track * 2);
	b_len += d.size() % (track * 2) / track;
	if (b_len > 1)
		insertion(d, d_pos, track, b_len);
}

static void pmergeme_recursive(std::deque<unsigned>& v, std::size_t d_pos[], unsigned track) {
	pairing(v, track);
...
static void pairing(std::deque<unsigned>& d, unsigned track) {
	unsigned	step = track * 2;
	std::size_t len = d.size() / step * step;
	for (unsigned i = track - 1; i < len; i += step)
		if (d[i] > d[i + track])
			std::swap_ranges(d.begin() + (i + 1 - track), d.begin() + (i + 1), d.begin() + (i + 1));
}
...
static void insertion(std::deque<unsigned>& d, std::size_t d_pos[], unsigned track,
	...
	for (std::size_t t = 0; t < b_len; k += 1, lb = t) {
		...
		if (t > b_len)
			t = b_len;
		for (std::size_t tb = t - 2; tb >= lb; tb -= 1)
			std::rotate(d.begin() + tb * 2 * track, d.begin() + (tb * 2 + 1) * track,
						d.begin() + (tb + t) * track);
		std::for_each(d_pos, d_pos + t - lb + 1, SteppingAssigner(lb * 2 * track, track * 2));
		std::size_t							   c_len = lb * 2;
		std::size_t							   d_len = t - lb - 1;
		std::deque<unsigned>::reverse_iterator rit_b = d.rend() - ((t - 1) * 2 + 1) * track;
		std::deque<unsigned>::reverse_iterator rit_e = rit_b + track;
		for (std::size_t tb = t - 1; tb >= lb; tb -= 1)
			binary_insertion(d, d_pos, track, rit_b, rit_e, tb - lb, c_len, d_len);
	}
}
